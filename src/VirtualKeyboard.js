...
class VirtualKeyboard extends Component {
		...
		return (
			<View style={[styles.container, this.props.style]}>
				...
				<View style={[styles.row, this.props.rowStyle]}>
					{this.halfCell('-')}
					{this.props.decimal ? this.halfCell('.') : <View style={{ flex: 1 }} /> }
					{this.Cell(0)}
					{this.Backspace()}
				</View>
			</View>
	...
	Backspace() {
		return (
			<TouchableOpacity accessibilityLabel='backspace' style={[styles.backspace,this.props.cellStyle,{backgroundColor:'transparent'}]} onPress={() => { this.onPress('back') }}>
	...
	halfCell(symbol) {
		return (
			<TouchableOpacity style={[styles.halfCell, this.props.cellStyle]} key={symbol} accessibilityLabel={symbol.toString()} onPress={() => { this.onPress(symbol.toString()) }}>
				<Text style={[styles.number, { color: this.props.color }]}>{symbol}</Text>
			</TouchableOpacity>
		);
	}
